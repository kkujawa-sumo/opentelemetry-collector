...
func Test_tracesamplerprocessor_SamplingProbabilityAttribute(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			traces:                           getTracesWithSpanWithAttribute(AttributeSamplingProbability, pdata.NewAttributeValueDouble(0.01)),
			wantSamplingProbabilityAttribute: pdata.NewAttributeValueDouble(0.01),
		},
		{
			name:                             "simple_with_invalid_attribute_value",
			traces:                           getTracesWithSpanWithAttribute(AttributeSamplingProbability, pdata.NewAttributeValueString("bar")),
			wantSamplingProbabilityAttribute: pdata.NewAttributeValueDouble(1.0),
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, td := range sink.AllTraces() {
				...
				attrValue, found := span.Attributes().Get(AttributeSamplingProbability)
